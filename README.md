...
```sh
git clone https://github.com/sham-bolic/dotfiles.git
cd dotfiles
```
...
- **Username**: run `./bootstrap.sh` (it detects your macOS username and offers to set it) OR change the single `user = "maxfong"` line in `flake.nix`.
...
- The `co` shell alias in `home.nix` runs `codex --approve-for-me`, which automatically reviews approval requests while retaining the workspace sandbox.
